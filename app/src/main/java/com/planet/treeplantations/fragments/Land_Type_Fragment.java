...
import com.planet.treeplantations.adapter.Land_Type_Adapter;
...
public class Land_Type_Fragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.fragment_land__type_, container, false);
        context=getActivity();
    ...
    private void user_info_API() {
        ...
        Call<Land_Type_Model> call = apiService.landType(preg_s.loadPreferences("userRole"));

        call.enqueue(new Callback<Land_Type_Model>() {

            @Override
            public void onResponse(Call<Land_Type_Model>call, Response<Land_Type_Model> response) {
                String user_info = response.body().getStatus();
                dialog.dismiss();
                List<Land_Typa_Responce_Model> land_typa_responce_models = response.body().getData();
                recyclerView.setAdapter(new Land_Type_Adapter(context,land_typa_responce_models));

            }

            @Override
            public void onFailure(Call<Land_Type_Model>call, Throwable t) {
                dialog.dismiss();
