...
public class FilteredArrayAdapter extends ArrayAdapter<Project_Responce_Model> {

    private List<Project_Responce_Model> objects;
    private Context context;
    private Filter filter;

    public FilteredArrayAdapter(Context context, int resourceId,	List<Project_Responce_Model> objects) {
    ...
    @Override
    public Project_Responce_Model getItem(int position) {
  ...
  /* @Override
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        try {
            Project_Responce_Model people = objects.get(position);
            if (people != null) {
    ...
    public Filter getFilter() {
        if (filter == null)
            filter = new AppFilter<Project_Responce_Model>(objects);
        return filter;
    }
        ...
        protected void publishResults(CharSequence constraint,
            ...
            if(filtered!=null && filtered.size()>0) {
                for (int i = 0, l = filtered.size(); i < l; i++)
                    add((Project_Responce_Model) filtered.get(i));
            }
